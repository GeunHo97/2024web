...
// BoardService7 - 기능별 각 함수로 구현
    ...
    // - 추가조건 : 등록 기능과 출력 기능 코드를 main 함수 밖으로 빼서 각 함수로 구현
    // - 분리 하는 이유 : 메소드는 기능 단위로 분리해서 하나의 함수로 묶으면 관리/사용/재활용 , 추후에 복잡한 프로젝트에서 유지보수/식별/구분 쉽다.
    //
...
// static main 함수 내에서 다른 함수가 실행되지 않는 이유( 다양한 이유 )
    ...
    //2. static main 함수 안에서 다른 클래스의 메소드를 호출하기 위해서는 객체가 필요하다
    //  -객체를 생성하면서 static이 아닌 메소드 정보를 읽어들인다.
public class BoardService7 { /* 실행클래스 */
