...
public class BoardService5 { /* 실행클래스 */
            ...
            if( choose == 1 ){
                ...
                for( int index = 0 ;  index <= boardList.length-1 ; index++ ){
                    if( boardList[index] == null ){ // 만약에 index번째 게시물이 비어 있으면
                        ...
                        board.setContent(content); board.setWriter(writer); board.setPassword(pwd);
                ...
                for( int index = 0 ; index <= boardList.length-1 ; index++ ){
                    if( boardList[index] != null ){ // 게시물이 존재하면
                        System.out.printf("작성자 : %s , 내용 : %s \n" ,
                                boardList[index].getWriter() , boardList[index].getContent() );
                    }
                } // for end
            }
        } // w end
